...
import { first, tap, map, filter, switchMap, delay, mergeMap } from "rxjs/operators";

import { cloneDeep, countBy, merge } from "lodash";
...
import { GenericConfirmationDialogComponent } from "@app/shared/genericConfirmationDialog/generic-confirmation-dialog.component";
...
export class PlayersListComponent implements OnInit {
  ...
  public isU24Taken$: Observable<boolean>;
  ...
  public ngOnInit(): void {
    ...
    this.players$ = this.store.data$.pipe(map((players) => players.filter((player) => !player.szrot)));
    ...
    this.availablePlayers$ = combineLatest(this.players$, this.selectedPlayers$).pipe(
      map(([players, selected]) =>
        players.filter((player) => selected.every((selection) => selection.name !== player.name))
      )
    );

    this.isUserSquadSent$ = this.store.currentSquad$.pipe(map((team) => !!team.length));
    ...
    this.isU24Taken$ = this.selectedPlayers$.pipe(
      map((selectedPlayers) => selectedPlayers.some((player) => player.u24))
    );
  ...
  public exportSquad(): void {
    ...
    } else if (this.store.getKsmValue() > this.maxKsm) {
  ...
  private prepareFiltering(): void {
    this.players$.pipe(first((players) => !!players.length)).subscribe((players) => {
  ...
  public disableSendSquadButton(): boolean {
    return (
      !!countBy(this.store.getSelectedPlayersValue(), "placeholder").true ||
      this.store.getKsmValue() > this.maxKsm ||
      new Date() > this.date
    );
  }
}

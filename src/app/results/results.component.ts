...
export class ResultsComponent implements OnInit {
  ...
  public ngOnInit(): void {
    this.store.options$
      .pipe(first((options) => !!options.currentRound))
      .subscribe((options) => this._chosenRoundSubject.next(options.currentRound));

    this.isUserSquadSent$ = this.chosenRound$.pipe(
      switchMap((round) => {
        return this.store.getRoundSquadsById(round, JSON.parse(localStorage.getItem("currentUser")).user.uid);
      }),
    ...
    this.squads$ = this.chosenRound$.pipe(
      ...
      switchMap((round) => {
        return combineLatest(this.store.getRoundSquads(round), this.store.getRoundScore(round), this.chosenRound$);
      }),
      map(([squads, scores, round]) => {
        ...
        playersResult.forEach((playerResult) => {
          ...
          if (playerResult.scoreSum) {
            this.store.setRoundResult(playerResult.userId, round, [playerResult.scoreSum, playerResult.bonusSum]);
          }
        });
  ...
  private _fetchStatsData(): void {
    this.statsData$ = this.chosenRound$.pipe(
      switchMap((round) => {
        return combineLatest(this.store.data$, this.store.getRoundScore(round), this.chosenRound$);
      }),
      tap(() => this._isLoadingSubject.next(true)),
  ...
  private _fetchTableData(): void {
    this.tableData$ = this.store.getRoundResult().pipe(
      ...
      map((results) => {
        ...
        // this.store.sendNewHistory(dataTable);

...
export class FilterPipe implements PipeTransform {
  ...
  transform(items: Player[], term: Filter): Player[] {
    ...
    if (term.showPossiblePlayers) {
      combineLatest(this.store.selectedPlayers$, this.store.ksmLeft$, this.store.options$)
        .pipe(take(1))
        .subscribe(([selected, ksmLeft, options]) => {
          const selection = countBy(
            selected.filter((item) => !item.placeholder),
            "type"
          );
          ...
          //     (player) => PlayerType.OBCOKRAJOWIEC.indexOf(player.type) === -1 || player.u24
          //   );
          // }
